...
- **2026-08-25 双产物制（用户反馈修正）**：Hermes 首跑的纯卷宗形态不符合用户所需——四项诊断全中（要报告不要只有卷宗、要理解不要尽调、缺综合结论、太长太密）。落地：产物改为 **report.md（给人读的主交付物：总览一屏/叙述体洞见/共识分歧/按用途分型章节/综合结论，~2000-3500 字）+ research.md（给 AI 的证据底座，schema 不变）**，同证据池派生、报告论断按编号回溯卷宗；新增 `[用途=理解|决策]` 参数（默认理解）；SKILL.md/report-template.md（新增）/dossier-template.md 头部/README 双语同步。
