...
若生成地图或课程需要联网研究，调用 `anything-research` 负责问题拆解和证据验证。新建或刷新公共地图时使用“领域制图模式、深度=深”，明确要求两条独立研究轨：深度研究领域的稳定知识结构，并深度研究截至当前日期的最新状态与变化。research 层自行选择底层联网工具；不要把“搜索到网页”当成研究完成，也不要在面向学习者的进度说明中暴露工具路由。需要说明时只说“由 anything-research 深度研究领域结构与最新变化并验证证据”，除非用户正在排查联网工具本身。

现有地图只有结构证据、缺少最新轨，或 `as_of` 相对该领域变化速度已经过期时，先刷新 research 卷宗并更新受影响地图节点，再建立学习契约。不能用一次 README/版本说明扫描替代最新信息深度研究。
